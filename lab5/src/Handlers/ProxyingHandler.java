    ...
    public void handle(SelectionKey key) throws Exception {
        ...
        ByteBuffer buffer;
            if (key.isReadable()) {
                    ...
                    if(remoteKey == null)
                    {
                        ...
                        try {
                            ...
                            if (count == -1) {
                                ...
                                LogWriter.logWorkflow("EOF is read in connection to host: " + operationHostIP + " .Connection will be closed", workflowLogger);
                                System.err.println(key.toString());
                                HandlerFactory.getChannelCloser().handle(key);
                            ...
                            LogWriter.logWorkflow("proxying: read " + buffer.position() + " bytes from host with address: " + operationHostIP, workflowLogger);
                            System.err.println(key.toString());
                    ...
                    {
                        LogWriter.logException(e, workflowLogger, "connection to the host:" + operationHostIP + "was emergency closed. ");
                        System.err.println(key.toString());
                        HandlerFactory.getChannelCloser().handle(key);
                    }
                    if (buffer.remaining() == 0) {
                        LogWriter.logWorkflow("proxying: wrote " + buffer.position() + " bytes to host with address: " + operationHostIP, workflowLogger);
                        System.err.println(key.toString());
                        if (remoteKey == null) {
                            // remote channel is closed, so close the connection
                            LogWriter.logWorkflow("Connection to the host" + remoteOperationHostIp + " was emergency closed. ", workflowLogger);
                            System.err.println(key.toString());
                            HandlerFactory.getChannelCloser().handle(key);
                        } else {
